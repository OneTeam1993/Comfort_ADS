            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        conn.Open();
                        cmd.Connection = conn;
                        cmd.CommandText = "INSERT INTO jobs (job_number, company, toc, timestamp, rx_time, company_id, reseller_id, asset_id, amount, bed, caller, destination, origin, phone, unit, accessories, remarks, remarks2, flag, patient, payment, trip, receipt_number, new_amount, agent, job_status, paramedic, driver_name, job_type) " +
                            "VALUES (@JobNumber, @Company, @TOC, @Timestamp, @RxTime, @AssetCompanyID, @AssetResellerID, @AssetID, @Amount, @Bed, @Caller, @Destination, @Origin, @Phone, @Unit, @Accessories, @Remarks, @Remarks2, @Flag, @Patient, @Payment, @Trip, @Receipt, @NewAmount, @Agent, @JobStatus, @Paramedic, @JobDriver, @JobType)";
                        cmd.Prepare();
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    {                       
                        ...
                        //Logger.LogEvent("GET THE LINKED Driver1: " + item.JobDriver + " GET THE LINKED Driver2: " + currJob.JobDriver, System.Diagnostics.EventLogEntryType.Warning);
                    }
                    catch (Exception ex)
                    {
