...
namespace FMSWebApi.Controllers
{
    ...
    public class JobListController : ApiController
    {
        ...
        public IEnumerable<JobInfo> GetJobList([FromUri]JobInfo param)
        {
            ...
            //if ((param.Timestamp != DateTime.MinValue && param.RxTime != DateTime.MinValue) && (param.AssetResellerID > 0 || param.AssetCompanyID > 0 || !string.IsNullOrEmpty(param.JobDriver)))  
            if ((param.Timestamp != DateTime.MinValue && param.RxTime != DateTime.MinValue) && (!string.IsNullOrEmpty(param.JobDriver)))         
            {
