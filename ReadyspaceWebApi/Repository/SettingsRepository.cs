...
namespace FMSWebApi.Repository
{
    public class SettingsRepository : ISettingsRepository
    {
        ...
        public IEnumerable<ComfortSettingsInfo> GetComfort()
        {
            ...
            currSettings.Host = "track-asia.com";
            currSettings.Port = 18026;
            currSettings.Timestamp = DateTime.Now.ToUniversalTime();
            currSettings.TimeInterval = 1;
            currSettings.AlertTitle = "ADS - Comfort Ambulance";
            currSettings.ErrorMessage = "Unable to process your request. Error Status: Parser error.";
            currSettings.EndpointURL = "https://track-asia.com/comfortwebapi/api/";
        ...
        public IEnumerable<RoyalSettingsInfo> GetRoyal()
        {
            ...
            currSettings.Host = "117.120.7.119";
            currSettings.Port = 18027;
            currSettings.TimeInterval = 30;
            currSettings.AlertTitle = "ADS - Royal Ambulance";
            currSettings.ErrorMessage = "Unable to process your request. Error Status: Parser error.";
            currSettings.EndpointURL = "http://117.120.7.119/comfortwebapi/api/";
            currSettings.User = "systemacecom";
            currSettings.Password = "1af372c8ea5e3190a178c93a1261024e2644b3a46b827f4ccd2de4aec4ba2ffc";
            currSettings.CountDown = 30;
            currSettings.LoginUsername = true;
        ...
        public IEnumerable<DemoSettingsInfo> GetDemo()
        {
            ...
            currSettings.Port = 18026;
            currSettings.TimeInterval = 30;
            currSettings.AlertTitle = "ADS - Demo";
            currSettings.ErrorMessage = "Unable to process your request. Error Status: Parser error.";
            currSettings.EndpointURL = "http://117.120.7.119/comfortwebapi/api/";
        ...
        //Distance Matrix
        public IEnumerable<DistanceMatrixInfo> GetDistanceMatrix(DistanceMatrixInfo param)
        {
            DistanceMatrixInfo currDistanceMatrix = new DistanceMatrixInfo();
            List<DistanceMatrixInfo> arrReverseGeocode = new List<DistanceMatrixInfo>();

            try
            {
                currDistanceMatrix.Origin = param.Origin;
                currDistanceMatrix.Destination = param.Destination;
                currDistanceMatrix.Rows = GetGoogleDistanceMatrix(currDistanceMatrix.Origin, currDistanceMatrix.Destination);
                arrReverseGeocode.Add(currDistanceMatrix);
            }
            catch (Exception ex)
            {
        ...
        private static string GetGoogleDistanceMatrix(string origin, string destination)
        {
            string retAddress = "";

            try
            {
                GoogleDistanceMatrixResponse revGeocodeRes = GoogleDistanceMatrix(origin, destination);
                if (revGeocodeRes.status == "OK")
                {
                    if (revGeocodeRes.rows != null)
                    {
            ...
            {
                Logger.LogEvent("GetGoogleDistanceMatrix Error: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
        ...
        private static GoogleDistanceMatrixResponse GoogleDistanceMatrix(string origin, string destination)
        {
            ...
            try
            {
                ...
                var urlRequest = String.Format("/maps/api/distancematrix/json?origins={0}destinations={1}&language={2}&client={3}", origin, destination, language, clientID);
