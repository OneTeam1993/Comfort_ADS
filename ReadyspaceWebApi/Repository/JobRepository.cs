...
namespace FMSWebApi.Repository
{
    ...
    {
        private string mConnStr = "server=localhost;uid=root;pwd=@c3c0M;database=comfort;Charset=utf8;max pool size=500;default command timeout=999999;";
        ...
        public IEnumerable<WardPickupInfo> GetAllJobWardOrigin()
        {
            MySqlConnection conn = new MySqlConnection();
            MySqlCommand cmd = new MySqlCommand();
            List<WardPickupInfo> arrJobs = new List<WardPickupInfo>();
        ...
        public IEnumerable<JobReportInfo> GetByCompanyReport(JobReportInfo param)
        {
            ...
            if (!string.IsNullOrEmpty(param.JobDriver)) query += " and driver_name = @JobDriver";
            if (!string.IsNullOrEmpty(param.JobType)) query += " and job_type = @JobType";
            if (!string.IsNullOrEmpty(param.Origin)) query += " and origin = @Origin";
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand(query, conn))
                    {
                        ...
                        if (!string.IsNullOrEmpty(param.Origin)) cmd.Parameters.AddWithValue("@Origin", param.Origin);
        ...
        public IEnumerable<WardInfo> GetByJobWard(JobReportInfo param)
        {
            ...
            if (!string.IsNullOrEmpty(param.Origin)) query2 += " and origin = @Origin";
        ...
        public IEnumerable<WardPickupInfo> GetByJobWardOrigin(JobReportInfo param)
        {

            List<WardPickupInfo> arrWard = new List<WardPickupInfo>();
            WardPickupInfo currWard = new WardPickupInfo();

            string query2 = "SELECT origin, count(origin) as countOrigin FROM view_jobs WHERE (timestamp between @StartTS and @EndTS)";
            ...
            query2 += " group by origin";
            query2 += " order by countOrigin desc";
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand(query2, conn))
                    {
                        ...
                        while (reader.Read())
                        {
                            currWard = DataMgrTools.BuildWardOrigin(reader);
                            arrWard.Add(currWard);
                        }
                        conn.Close();
                    }
                ...
                {
                    Logger.LogEvent(mProjName, "GetByJobWardOrigin: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                }
            }
        ...
        public string GetImage(string strName, ref string strFill)
        {
            try
            {
                ...
                foreach (string strType in arrTypes)
                {
                    ...
                    string strUri = String.Format("{0}/images/{1}", "http://117.120.7.119/comfortwebapi", strTemp);
