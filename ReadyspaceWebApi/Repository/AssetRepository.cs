...
namespace FMSWebApi.Repository
{
    public class AssetRepository : IAssetRepository
    {
        ...
        public AssetInfo GetByImeiName(string tagName)
        {
            AssetInfo currAsset = new AssetInfo();
            string query = string.Format("SELECT * FROM view_assets_imei WHERE imei = @IMEI");
            ...
            using (MySqlConnection conn = new MySqlConnection(mConnStr))
            {
                try
                {
                    using (MySqlCommand cmd = new MySqlCommand())
                    {
                        ...
                        cmd.CommandText = query;
                        cmd.Prepare();
                        cmd.Parameters.AddWithValue("@IMEI", tagName);
                        MySqlDataReader reader = cmd.ExecuteReader();

                        while (reader.Read())
                        {
                            currAsset = DataMgrTools.BuildAssetImei(reader);
                        }
                        conn.Close();
                    }
                }
                ...
                {
                    Logger.LogEvent(ex.Message + "-GetByImeiName", System.Diagnostics.EventLogEntryType.Error);
                }
            }
            return currAsset;
        }
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    try
                    {
                        ...
                        //Logger.LogEvent("GET JobDriver: " + currDriver.Name, System.Diagnostics.EventLogEntryType.Warning);
                    ...
                    try
                    {
                        using (MySqlCommand cmd = new MySqlCommand())
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.CommandText = "UPDATE jobs SET asset_id = @AssetID WHERE timestamp between @StartTS and @EndTS and driver_name = @JobDriver and job_status = @JobStatus";
                            cmd.Prepare();
                            cmd.Parameters.AddWithValue("@StartTS", DateTime.Today);
                            cmd.Parameters.AddWithValue("@EndTS", DateTime.Today.AddDays(1));
                            cmd.Parameters.AddWithValue("@JobDriver", currDriver.Name);
