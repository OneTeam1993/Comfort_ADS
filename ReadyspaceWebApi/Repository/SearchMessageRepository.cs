...
namespace FMSWebApi.Repository
{
    public class SearchMessageRepository : ISearchMessageRepository
    {
        ...
        private string mConnStr = "server=localhost;uid=root;pwd=@c3c0M;database=comfort;default command timeout=999999;";
        ...
        public SearchMessage Add(SearchMessage currMessage)
        {
            try
            {
                if (!string.IsNullOrEmpty(currMessage.Recipients))
                {
                    using (MySqlConnection conn = new MySqlConnection(mConnStr))
                    {
