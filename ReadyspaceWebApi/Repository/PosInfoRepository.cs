...
namespace FMSWebApi.Repository
{
    public class PosInfoRepository : IPosRepository
    {
        ...
        public PosInfo Add(PosInfo item)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    if (item.PosY != 0 && item.PosX != 0) VehTools.ApplyReverseGeocode(currAsset, item, null);
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    try
                    {
                        using (MySqlCommand cmd = new MySqlCommand())
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.CommandText = "SELECT * FROM positions_cvr WHERE asset_id = @AssetID AND engine IN (0,1) AND rx_time IS NULL LIMIT 1";
                            cmd.Prepare();
                            cmd.Parameters.AddWithValue("@AssetID", udtPrev.AssetID);
                    ...
                    else
                    {
                        ...
                        if (udtPos.Engine == "IDLE")
                        {
                            ...
                            {
                                Logger.LogEvent(mProjName, " save idle data cvr: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                            }
        ...
        public string SaveMoveReport(PosInfo udtPrev, PosInfo udtPos)
        {
            string strRet = "";
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    if (udtPos.Engine == "MOVE")
                    {
                        ...
                        {
                            Logger.LogEvent(mProjName, " save move data cvr: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                        }
            ...
            {
                // log error
                Logger.LogEvent(mProjName, "SaveMoveReport: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
