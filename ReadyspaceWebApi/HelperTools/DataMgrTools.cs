...
namespace FMSWebApi
{
    ...
    {
        private static string mProjName = "Comfort";
        ...
        public static AssetInfo BuildAssetImei(MySqlDataReader dbRdr)
        {
            AssetInfo udtAsset = new AssetInfo();
            AssetRepository assetRepo = new AssetRepository();
            ...
            try
            {
                ...
                udtAsset.Flag = dbRdr.ToString("asset_remarks");
                udtAsset.Imei = dbRdr.ToString("imei");
        ...
        public static JobReportInfo BuildJobReport(MySqlDataReader dbRdr)
        {
            JobReportInfo udtJobs = new JobReportInfo();
            try
            {
                // set jobs parameters
                udtJobs.JobID = dbRdr.ToInt64("job_id");
        ...
        public static GenerateJobID BuildGenerateJobID(MySqlDataReader dbRdr)
        {
            GenerateJobID udtJobs = new GenerateJobID();
            try
            {
                // set jobs parameters
                udtJobs.NewJobID = dbRdr.ToInt64("NewJobID");
            }
            catch (Exception ex)
            {
                // log error
                Logger.LogEvent(mProjName, "BuildGenerateJobID ERROR: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
            return udtJobs;
        }
        ...
        public static HospitalInfo BuildHospital(MySqlDataReader dbRdr)
        {
            HospitalInfo udtHospital = new HospitalInfo();
            try
            {

                // set users parameters
                udtHospital.HospitalID = dbRdr.ToInt32("hospital_id");
                udtHospital.Name = dbRdr.ToString("name");
                udtHospital.Address = dbRdr.ToString("address");
                udtHospital.ShortName = dbRdr.ToString("short_name");
                udtHospital.CodeID = dbRdr.ToInt32("code_id");

            }
            catch (Exception ex)
            {
                // log error
                Logger.LogEvent(mProjName, "BuildHospital ERROR: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
            return udtHospital;
        }


        public static WardInfo BuildWard(MySqlDataReader dbRdr)
        {
            WardInfo udtWard = new WardInfo();
            try
            {

                // set users parameters
                
                udtWard.Destination = dbRdr.ToString("destination");
                udtWard.CountDestination = dbRdr.ToInt32("countDestination");

            }
            catch (Exception ex)
            {
                // log error
                Logger.LogEvent(mProjName, "BuildWard ERROR: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
            return udtWard;
        }
        public static WardPickupInfo BuildWardOrigin(MySqlDataReader dbRdr)
        {
            WardPickupInfo udtWard = new WardPickupInfo();
            try
            {

                // set users parameters

                udtWard.Origin = dbRdr.ToString("origin");
                udtWard.CountOrigin = dbRdr.ToInt32("countOrigin");

            }
            catch (Exception ex)
            {
                // log error
                Logger.LogEvent(mProjName, "BuildWard Origin ERROR: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
            }
            return udtWard;
        }
